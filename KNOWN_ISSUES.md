...
- **T13** /redesign 実寸監査(2026-08-03 登録・**実行は別途指示**)。master study の換算知見(`docs/design-notes-master-study.md`)に基づき、タイポスケール・余白・コントロール寸法を実機基準(402pt・実測)で校正する。**ただし 1px line 文法・radius 体系(13/10/6)・書体は N2 のまま変更しない**。**2026-08-05 注記: 本タスクは骨格統合実験 `/redesign-v2`(design/skeleton-n2 ブランチ)に吸収** — 監査知見は同モックへ直接適用した
